...
help:
	...
	@echo "  make backup        - Hot backup Redis data dir via BGSAVE (OUTPUT=path/to/dir)"
	@echo "  make restore       - Cold restore Redis data dir (INPUT=path/to/dir)"
	@echo "  make backup-json   - Dump all memories to JSON (OUTPUT=path/to/file.json)"
...
backup:
	@if [ -z "$(OUTPUT)" ]; then \
		echo "Usage: make backup OUTPUT=path/to/backup-dir"; \
		exit 1; \
	fi
	@$(CURDIR)/scripts/backup-native.sh $(OUTPUT)
...
backup-json: build
	@if [ -z "$(OUTPUT)" ]; then \
		echo "Usage: make backup-json OUTPUT=path/to/backup.json"; \
		exit 1; \
	fi
	$(BIN_DIR)/$(BINARY) backup --config $(CONFIG_DIR)/config.toml --output $(OUTPUT)
